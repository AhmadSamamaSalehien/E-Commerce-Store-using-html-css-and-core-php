...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_size'])) {
    ...
    if ($conn->query("INSERT INTO sizes (name) VALUES ('$name')")) {
        $_SESSION['toast_message'] = 'Size added successfully';
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_size'])) {
    ...
    if ($conn->query($sql)) {
        $_SESSION['toast_message'] = 'Size updated successfully';
    } else {
...
if (isset($_GET['delete_id'])) {
    ...
    $check_query = $conn->query("SELECT COUNT(*) as count FROM products WHERE size_id = $id");
    $row = $check_query->fetch_assoc();
    if ($row['count'] > 0) {
        $_SESSION['toast_message'] = 'Cannot delete size because it is associated with ' . $row['count'] . ' product(s)';
    } else {
        if ($conn->query("DELETE FROM sizes WHERE id = $id")) {
            $_SESSION['toast_message'] = 'Size deleted successfully';
        } else {
            $_SESSION['toast_message'] = 'Failed to delete size';
        }
    }
    header("Location: admin_sizes.php");
    exit;
...
<head>
    ...
    <title>Manage Sizes - My Store</title>
        ...
        <div id="main">
                ...
                <div class="page-title">
                    <div class="row">
                        <div class="col-12 col-md-6 order-md-1 order-last">
                            <h3><i class="bi bi-arrows-angle-contract"></i> Manage Sizes</h3>
                            <p class="text-subtitle text-muted">Add, edit, or remove sizes</p>
                        </div>
                                ...
                                <div class="modal-body">
                                    ...
                                    <form method="POST" class="row g-3">
                                        <div class="col-12">
                                            ...
                                            <input type="text" name="name" id="name" class="form-control" placeholder="Size Name" required>
                                        </div>
                                        <div class="col-12">
                                            <button type="submit" name="add_size" class="btn btn-primary"><i class="bi bi-plus-circle"></i> Add Size</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                                ...
                                <div class="modal-body">
                                    ...
                                    <form method="POST" class="row g-3">
                                        ...
                                        <div class="col-12">
                                            ...
                                            <input type="text" name="name" id="edit_name" class="form-control" placeholder="Size Name" required>
                                        </div>
                                        <div class="col-12">
                                            <button type="submit" name="edit_size" class="btn btn-primary"><i class="bi bi-save"></i> Save Changes</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                                    ...
                                    <tbody>
                                        <?php while ($row = $result->fetch_assoc()): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <button class="btn btn-primary btn-sm edit-size-btn" 
                                                            data-bs-toggle="modal" 
                                                            data-bs-target="#editSizeModal" 
                                                            data-size-id="<?php echo $row['id']; ?>" 
                                                            data-size-name="<?php echo htmlspecialchars($row['name']); ?>">
                                                        <i class="bi bi-pencil"></i> Edit
                                                    </button>
                                                    <button class="btn btn-danger btn-sm delete-size-btn" 
                                                            data-bs-toggle="modal" 
            ...
            <div class="modal fade" id="deleteSizeModal" tabindex="-1" aria-labelledby="deleteSizeModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="deleteSizeModalLabel">Confirm Deletion</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            Are you sure you want to delete the size "<span id="size-name"></span>"?
                        </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            deleteButtons.forEach(button => {
                button.addEventListener('click', function () {
                    ...
                    const sizeNameSpan = document.getElementById('size-name');

                    // Set the size name in the modal
                    sizeNameSpan.textContent = sizeName;
                    // Set the delete URL in the confirm button
                    confirmDeleteBtn.setAttribute('href', `?delete_id=${sizeId}`);
                });
            });
